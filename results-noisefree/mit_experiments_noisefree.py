...
To run this file, use:
  python mit_experiments_noisefree.py <L> <n_trials> <folding_fn> <scale_factors>
where
...
The following file containing the optimal variational is required:
...
from fidelity_susceptibility import (
    compute_second_energy_derivative,
    compute_fidelity_susceptibility,
)

from zne import zne, richardson_extrapolation
...
def parse_command_line_arguments(args):
    ...
    if len(args) != 5:
        ...
        print(f"python mit_experiments_noisefree.py <L> <n_trials> <folding_fn> <scale_factors>")
        sys.exit()
...
if __name__ == "__main__":
    ...
    if L == 4:
        ...
        from spins_4 import reduced_H_1, reduced_H, variational_ansatz
    if L == 6:
        initial_layout = [2, 3, 4, 0, 1] # Next lowest CNOT error between qubits 2/3 so use those
        n_qubits = 3
        n_params = 7
        from spins_6 import reduced_H_1, reduced_H
        from spins_6 import variational_ansatz_manila as variational_ansatz # Optimized for arch
